...
export const apiWaypointCreate = new class ApiWaypointCreate {
    create(player, info, showMessage = true, isShare = "no") {
        const color = colorRBG[info.color] ?? { red: 255, green: 255, blue: 255 };
        ...
        if (isShare != "no")
            apiWarn.notify(player, { translate: "advanced_waypoints.warn.reciveWaypoint", with: [name, isShare] }, { sound: "advanced_waypoints.warn.enchanting_table" });
        if (info.visible)
            apiWaypointEntity.spawn(player, { id: name, dim: player.dimension.id, pos: apiVec3.center(info.pos), icon: info.icon, rgb: color });
    }
};
