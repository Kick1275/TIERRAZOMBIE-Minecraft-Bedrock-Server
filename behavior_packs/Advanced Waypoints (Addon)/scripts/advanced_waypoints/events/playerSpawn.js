...
world.afterEvents.playerSpawn.subscribe(({ initialSpawn, player }) => {
    if (!initialSpawn) {
        ...
        if (!player.getDynamicProperty("aw:7.0"))
            return convert_v7_0(player);
    }
});
function convert_v7_0(player) {
    ...
    for (const dynamic of allDynamic) {
        if (!dynamic.startsWith("waypointList"))
            continue;
        const info = player.getDynamicProperty(dynamic);
        if (typeof info != "string")
            continue;
        const waypoints = JSON.parse(info);
        for (const way of waypoints)
            player.setDynamicProperty(`${way.world.replace("minecraft:", "")}/${way.visible ? 1 : 0}/${way.icon == 7 ? 1 : 0}/${way.rgb.r},${way.rgb.g},${way.rgb.b}/${Math.floor(way.pos.x)},${way.pos.y},${Math.floor(way.pos.z)}/${way.id}`, true);
