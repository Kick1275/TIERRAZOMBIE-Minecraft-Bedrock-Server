...
export const waypointsLocations = new Map<string, Vector3>()
export const lastPositions = new Map<string, number>()
  ...
  for(const player of world.getAllPlayers()){
    ...
    for(const entity of entities){
      try{
        ...
        if(!entity.getTags().includes(`waypointWorld: ${player.dimension.id.replace("minecraft:", "")}`)){ apiWaypointEntity.remove(player, entity); continue }

        const distance = Math.floor(apiVec3.distance3(playerPos, waypointPos))
        if(lastPositions.get(entity.id) == distance) continue
        ...
        if(config.dis) apiWaypointEntity.setDisplayDistance(entity, distance)
...
function getWaypointPos(entity: Entity): Vector3 {
...
export function offset(pointA: Vector3, pointB: Vector3): Vector3 {
  const distance = apiVec3.distanceXYZ(pointA, pointB)

  const length = Math.sqrt(distance.x **2 + distance.y **2 + distance.z **2)

  if (length === 0) return { x: pointA.x, y: pointA.y, z: pointA.z }
