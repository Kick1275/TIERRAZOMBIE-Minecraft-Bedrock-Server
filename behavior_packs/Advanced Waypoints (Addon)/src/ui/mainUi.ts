...
import { apiWaypointInfo, WaypointInfo } from "../lib/apiwaypoint/info"
...
export function waypointMenu(player: Player, quickCreate: boolean){
  if(quickCreate) return waypointsUIController[1]?.(player)

  const form = new ActionFormData()
  .title("advanced_waypoints.ui.config.title")
  if(enableTp()) form.button("advanced_waypoints.ui.config.button.teleport", "textures/ui/realmsIcon")
...
const waypointsUIController = new class WaypointsUIController {
  [key: number]: (player: Player, edit?: WaypointEdit) => void

  0(player: Player): void { waypointUi.teleport(player) }
  1(player: Player): void { waypointUi.create(player) }
  2(player: Player): void { waypointUi.edit(player) }
  3(player: Player): void { waypointUi.remove(player) }
  4(player: Player): void { waypointUi.share(player) }
  5(player: Player): void { waypointUi.options(player) }
}
...
const waypointUi = new class WaypointUi {
  create(player: Player, edit?: WaypointEdit): void {
    ...
    .show(player).then(r => {
      if(r.canceled || r.formValues == undefined) return edit ? this.edit(player) : apiWarn.notify(player, "advanced_waypoints.warn.cancelCreatePoint", {type: "action_bar", "sound": "advanced_waypoints.warn.break"})

      const [ name, position, icon, color, colorR, colorG, colorB ] = r.formValues

      if(typeof name != "string" || !name) return apiWarn.notify(player, "advanced_waypoints.warn.invalidName", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})
      if(typeof position != "string" || !name) return apiWarn.notify(player, "advanced_waypoints.warn.invalidName", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})

      const xyz = position.split(/[^0-9.,-]+/).filter(p => p !== "").map(p => parseFloat(p.replace(',', '.'))).filter(n => !isNaN(n)).map(n => Math.trunc(n))
      if(xyz.length < 3) return apiWarn.notify(player, "advanced_waypoints.warn.invalidPos", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})

      const pos = {x: xyz[0], y: xyz[1] ? xyz[1] +1 : xyz, z: xyz[2]}
      if(!apiVec3.isValid(pos)) return apiWarn.notify(player, "advanced_waypoints.warn.invalidPos", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})

      if(typeof icon != "number") return apiWarn.notify(player, "advanced_waypoints.warn.failCreatePoint", {type: "action_bar", sound: "advanced_waypoints.warn.break"})
      if(typeof color != "number") return apiWarn.notify(player, "advanced_waypoints.warn.failCreatePoint", {type: "action_bar", sound: "advanced_waypoints.warn.break"})
  ...
  edit(player: Player): void {
    ...
    if(waypoints.length < 1) return apiWarn.notify(player, "advanced_waypoints.warn.dontHavePoints", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})
    ...
    form.show(player).then(r => {
      ...
      if(r.selection == 0) return this.visible(player)
  ...
  visible(player: Player): void {
    ...
    .show(player).then(({canceled, formValues}) => {
      ...
      for(const value of formValues){
        ...
        if(typeof value == "boolean" && value) amountVisible++
      }
      ...
      for(let i = 0; i < formValues.length; i++){
        ...
        if(visibility) apiWaypointEntity.spawn(player, {...info, icon: info.icon, rgb: info.rgb})
      }
    })
  }
  ...
  remove(player: Player): void {
    const waypoints = apiWaypointInfo.getAll(player)
    if(waypoints.length < 1) return apiWarn.notify(player, {translate: "advanced_waypoints.warn.dontHavePoints"}, {type: "action_bar", sound: "advanced_waypoints.warn.bass"})
    ...
    form.show(player).then(({canceled, selection}) => {
      ...
      if(!point) return apiWarn.notify(player, "advanced_waypoints.warn.waypointNotFound", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})
      ...
      .show(player).then(r2 => {
        if(r2.canceled || r2.selection == 1) return apiWarn.notify(player, "advanced_waypoints.warn.dontDelete", {type: "action_bar", sound: "advanced_waypoints.warn.orb"})
  ...
  teleport(player: Player): void {
    ...
    form.show(player).then(r => {
      ...
      if(player.level < button.cost && player.getGameMode() != "creative") return apiWarn.notify(player, "advanced_waypoints.warn.insufficientXp", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})
  ...
  share(player: Player): void {
    ...
    if(allPlayers.length < 1) return apiWarn.notify(player, "advanced_waypoints.warn.noPlayersOnline", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})

    const allPoints = apiWaypointInfo.getAll(player)
    if(allPoints.length < 1) return apiWarn.notify(player, "advanced_waypoints.warn.dontHavePoints", {type: "action_bar", sound: "advanced_waypoints.warn.bass"})
    ...
    form1.show(player).then(({canceled, selection}) => {
      if(canceled || selection == undefined) return apiWarn.notify(player, "advanced_waypoints.warn.dontShared", {type: "action_bar"})
      ...
      form2.show(player).then(({canceled: canceled2, selection: targetIndex}) => {
        ...
        if(!targetConfig.share) return apiWarn.notify(player, "advanced_waypoints.warn.targetDontAcceptShare", {sound: "advanced_waypoints.warn.break"})
...
interface WaypointEdit extends WaypointInfo {
  index: number
}