...
export const waypointOptions = new class waypointOptions {
  ...
  0(player: Player): void {
    ...
    .show(player).then(r => {
      if(r.canceled || r.formValues == undefined) return apiWarn.notify(player, "advanced_waypoints.warn.cancelOptions", {type: "action_bar", sound: "advanced_waypoints.warn.break"})
