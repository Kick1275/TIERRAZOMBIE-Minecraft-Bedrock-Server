...
import { WaypointInfo } from "./info"
import { sameNames } from "./format"

export const apiWaypointEdit = new class ApiWaypointEdit {
  edit(player: Player, oldInfo: WaypointEdit, info: WaypointInfoEdit, sound = true): void {
    apiWaypointInfo.remove(player, oldInfo.id)
    apiWaypointEntity.remove(player, oldInfo.id)
    if(oldInfo.id != info.name) apiWaypointShare.removeFromList(player, oldInfo.id)
    ...
    const color: RGB = info.color != oldColor ? {...(colorRBG[info.color] ?? {red: 255, green: 255, blue: 255})} : {red: info.colorR, green: info.colorG, blue: info.colorB}
    ...
    if(info.visible) apiWaypointEntity.spawn(player, {id: name, dim: oldInfo.dim, pos: pos, icon: info.icon, rgb: {red: color.red, green: color.green, blue: color.blue}})
