...
export const apiWaypointInfo = new class ApiWaypointInfo {
  getAll(player: Player): WaypointInfo[] {
    const waypointsInfo = player.getDynamicPropertyIds()

    const waypoints: WaypointInfo[] = []
    for(const allInfo of waypointsInfo){
      if(allInfo.startsWith("aw:") || !allInfo.startsWith(player.dimension.id.replace("minecraft:", ""))) continue
  ...
  remove(player: Player, name: string): void {
    ...
    if(waypoints) player.setDynamicProperty(waypoints, undefined)
  }
}
